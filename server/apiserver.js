...
const saveCache = (post_cache_header,foodtype) => {      
  ...
  const getFood = (options,message,size,foodtype) => {
    ...
    if(responsejson.results.length) //Object.keys(responsejson).length
    {
        ...
        if(obj.name.toLowerCase().includes(options.params.q.toLowerCase()) && (!!obj.description || obj.description.length > 0) && !obj.description.startsWith(' '))
        {
          ...
          //map_index++;             
              ...
              fooddescriptionarrres[arr_i] = [];  
                  ...
                  {                  
                    fooddescriptionarrres[arr_i][arr_j] = helper.calculateElement(obj,arr_j,inst);
                  }
                  arr_i++;            
        }
      ...
      switch(foodtype)
      {
        case Enums.DESC : helper.printFood(map_desc,message,options); break;
        case Enums.VID : helper.printFood(map_vid,message,options); break;
        case Enums.THUMB : helper.printFood(map_thumb,message,options); break;
        case Enums.DISP : helper.printFood(map_display,message,options); break;
        case Enums.INST : helper.printFood(map_inst,message,options); break;
      }
    }
